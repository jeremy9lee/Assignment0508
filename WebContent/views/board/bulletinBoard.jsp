			...
			<center><h2>자유게시판</h2></center>
				<ul>
					<li>
								...
								<c:when test="${not empty listByAdmin }">
									<c:forEach items="${listByAdmin}" var="b">
										<tr style="font-weight: bold">
											<td>${b.b_no}</td>
											<td><a href = "bulletin?a=bulletinDetail&no=${b.b_no}"><strong>${b.b_title}</strong></a></td>
											<td><b>${b.name}</b></td>
											<td>${b.reg_date}</td>
											<td>${b.hit }</td>
										</tr>
									...
									<c:forEach items="${list}" var="b">
										<tr>
											<td>${b.b_no}</td>
											<td><a href = "bulletin?a=bulletinDetail&no=${b.b_no}">${b.b_title}</a></td>
											<td>${b.name}</td>
											<td>${b.reg_date}</td>
											<td>${b.hit }</td>
										</tr>
						...
						<a href = "bulletin?a=bulletinWriteForm"><input type = "button" value = "write" class="btn btn-info"></a>
						</div>
